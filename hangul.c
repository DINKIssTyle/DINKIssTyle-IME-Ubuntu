...
void dkst_hangul_free(DKSTHangul *h) {
    if (h->buffer) g_string_free(h->buffer, TRUE);
    if (h->completed) g_string_free(h->completed, TRUE);
}
...
static uint32_t map_key(char c) {
    switch(c) {
        ...
        case 'o': return 0x1162; case 'O': return 0x1164; // ㅐ, ㅒ
        case 'p': return 0x1166; case 'P': return 0x1168; // ㅔ, ㅖ
...
static uint32_t compatibility_jamo(uint32_t u) {
    if (0x1100 <= u && u <= 0x1112) {
        ...
        int idx = u - 0x1100;
        if (idx >= 0 && idx < 19) return map[idx];
    }
    if (0x1161 <= u && u <= 0x1175) {
        ...
        int idx = u - 0x1161;
        if (idx >= 0 && idx < 21) return map[idx];
    }
    return u;
}

static int cho_index(uint32_t c) {
    if (0x1100 <= c && c <= 0x1112) return c - 0x1100;
    return -1;
}
static int jung_index(uint32_t c) {
    if (0x1161 <= c && c <= 0x1175) return c - 0x1161;
    return -1;
}
static int jong_index(uint32_t c) {
    if (0x11A8 <= c && c <= 0x11C2) return c - 0x11A8 + 1;
...
static uint32_t cho_to_jong(uint32_t c) {
    ...
    switch(c) {
        case 0x1100: return 0x11A8; case 0x1101: return 0x11A9; case 0x1102: return 0x11AB;
        case 0x1103: return 0x11AE; case 0x1105: return 0x11AF; case 0x1106: return 0x11B7;
        case 0x1107: return 0x11B8; case 0x1109: return 0x11BA; case 0x110A: return 0x11BB;
        case 0x110B: return 0x11BC; case 0x110C: return 0x11BD; case 0x110E: return 0x11BE;
        case 0x110F: return 0x11BF; case 0x1110: return 0x11C0; case 0x1111: return 0x11C1;
...
static uint32_t jong_to_cho(uint32_t c) {
     switch(c) {
        case 0x11A8: return 0x1100; case 0x11A9: return 0x1101; case 0x11AB: return 0x1102;
        case 0x11AE: return 0x1103; case 0x11AF: return 0x1105; case 0x11B7: return 0x1106;
        case 0x11B8: return 0x1107; case 0x11BA: return 0x1109; case 0x11BB: return 0x110A;
        case 0x11BC: return 0x110B; case 0x11BD: return 0x110C; case 0x11BE: return 0x110E;
        case 0x11BF: return 0x110F; case 0x11C0: return 0x1110; case 0x11C1: return 0x1111;
...
static void split_jung(uint32_t c, uint32_t *j1, uint32_t *j2) {
    *j1 = c; *j2 = 0;
    if (c == 0x116A) { *j1 = 0x1169; *j2 = 0x1161; }
    else if (c == 0x116B) { *j1 = 0x1169; *j2 = 0x1162; }
    else if (c == 0x116C) { *j1 = 0x1169; *j2 = 0x1175; }
    else if (c == 0x116F) { *j1 = 0x116e; *j2 = 0x1165; }
    else if (c == 0x1170) { *j1 = 0x116e; *j2 = 0x1166; }
    else if (c == 0x1171) { *j1 = 0x116e; *j2 = 0x1175; }
    else if (c == 0x1174) { *j1 = 0x1173; *j2 = 0x1175; }
}

static uint32_t combine_jong(uint32_t a, uint32_t b) {
    if (a == 0x11A8 && b == 0x11BA) return 0x11AA; // ㄳ
    if (a == 0x11AB && b == 0x11BD) return 0x11AC; // ㄵ
    if (a == 0x11AB && b == 0x11C2) return 0x11AD; // ㄶ
    if (a == 0x11AF && b == 0x11A8) return 0x11B0; // ㄺ
    if (a == 0x11AF && b == 0x11B7) return 0x11B1; // ㄻ
    if (a == 0x11AF && b == 0x11B8) return 0x11B2; // ㄼ
    if (a == 0x11AF && b == 0x11BA) return 0x11B3; // ㄽ
    if (a == 0x11AF && b == 0x11C0) return 0x11B4; // ㄾ
    if (a == 0x11AF && b == 0x11C1) return 0x11B5; // ㄿ
    if (a == 0x11AF && b == 0x11C2) return 0x11B6; // ㅀ
    if (a == 0x11B8 && b == 0x11BA) return 0x11B9; // ㅄ
    return 0;
}

static void split_jong(uint32_t c, uint32_t *j1, uint32_t *j2) {
    *j1 = c; *j2 = 0;
    if (c == 0x11AA) { *j1 = 0x11A8; *j2 = 0x11BA; }
    else if (c == 0x11AC) { *j1 = 0x11AB; *j2 = 0x11BD; }
    else if (c == 0x11AD) { *j1 = 0x11AB; *j2 = 0x11C2; }
    else if (c == 0x11B0) { *j1 = 0x11AF; *j2 = 0x11A8; }
    else if (c == 0x11B1) { *j1 = 0x11AF; *j2 = 0x11B7; }
    else if (c == 0x11B2) { *j1 = 0x11AF; *j2 = 0x11B8; }
    else if (c == 0x11B3) { *j1 = 0x11AF; *j2 = 0x11BA; }
    else if (c == 0x11B4) { *j1 = 0x11AF; *j2 = 0x11C0; }
    else if (c == 0x11B5) { *j1 = 0x11AF; *j2 = 0x11C1; }
    else if (c == 0x11B6) { *j1 = 0x11AF; *j2 = 0x11C2; }
    else if (c == 0x11B9) { *j1 = 0x11B8; *j2 = 0x11BA; }
}

uint32_t dkst_hangul_current_syllable(DKSTHangul *h) {
    if (h->cho == 0 && h->jung == 0 && h->jong == 0) return 0;
    
    // Independent Jamo
    if (h->cho && !h->jung && !h->jong) return compatibility_jamo(h->cho);
    if (!h->cho && h->jung && !h->jong) return compatibility_jamo(h->jung);
    ...
    // Moa-jjik-gi partial
    if (j != -1 && c == -1) return compatibility_jamo(h->jung);
    
    return 0; // Should not happen with valid logic 
}

bool dkst_hangul_backspace(DKSTHangul *h) {
    if (h->cho == 0 && h->jung == 0 && h->jong == 0) return false;
...
bool dkst_hangul_process(DKSTHangul *h, char key) {
    ...
    if (IS_CHO(hangul)) {
        ...
        if (h->jong) {
            ...
            split_jong(h->jong, &j1, &j2);
            if (j2) {
                // Complex jong. Split it. current becomes Cho+Jung+J1. Next is J2(as Cho)+NewJung
                h->jong = j1;
                uint32_t syl = dkst_hangul_current_syllable(h);
                append_unichar(h->completed, syl);
...
char* dkst_hangul_get_commit_string(DKSTHangul *h) {
    if (h->completed->len == 0) return NULL;
